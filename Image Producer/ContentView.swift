...
struct ContentView: View {
    ...
    /// selecting another layer. On commit the original placement is preserved as a hidden
    /// copy directly BELOW the committed one, matching the app's standing rule that every
    /// destructive Apply keeps its source (`ImageDocument.addResultLayer`).
    ///
    /// Michael 2026-09-03: "manipulate the layer freely and when the user changes tool or
    /// switches to a new layer i want the move/transform edits to become destructive and
    /// the original unedited layer copy hidden below the commited copy with a new nane
    /// {original layer name} {(tool name)}".
    ///
    /// This is Photoshop's Free Transform shape — adjust live, then commit — with the tool
    ...
    private var canvasArea: some View {
        ...
        // Selecting a different layer commits the one you were placing.
        .onChange(of: activeLayerID) {
            commitMoveSession()
            beginMoveSessionIfNeeded()
        }
        // Cover the launch case: Move is the default tool, so a layer can be active and
        // draggable before either change above has ever fired.
        .onAppear { beginMoveSessionIfNeeded() }
    }

    // MARK: - Move/Transform session (live edit, committed on leave)

    /// Remember the placement a layer had BEFORE the user starts moving it. No-op unless
    /// the Move tool is active with a layer selected.
    private func beginMoveSessionIfNeeded() {
        guard activeTool == .move, let id = activeLayerID,
              let layer = document.layers.first(where: { $0.id == id }) else {
            moveSessionLayerID = nil
            moveSessionTransform = nil
            return
        }
        // MUST happen before any manipulation — otherwise "Original" in the History panel
        // would record the already-moved state.
        document.captureHistoryBaselineIfNeeded()
        moveSessionLayerID = id
        moveSessionTransform = layer.transform
    }

    /// Commit the open Move session: keep the moved layer where it is, rename it with the
    /// tool's suffix, and slip an unedited hidden copy in directly beneath it.
    private func commitMoveSession() {
        defer {
            moveSessionLayerID = nil
            moveSessionTransform = nil
        }
        guard let id = moveSessionLayerID,
              let before = moveSessionTransform,
              let i = document.layers.firstIndex(where: { $0.id == id }) else { return }
        // Nothing actually moved — selecting a layer and leaving it alone must not litter
        ...
        var base = document.layers[i].name
        if let r = base.range(of: #" \(Move(?: \d+)?\)$"#, options: .regularExpression) {
        ...
        document.recordHistory(toolID: Tool.move.rawValue,
                               groupTitle: Tool.move.title,
                               actionLabel: "Move",
                               layerID: committedID)
    }
